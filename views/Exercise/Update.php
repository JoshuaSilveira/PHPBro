...
$is_valid = true;
$name = $name_err = $bodyweight_err = $cardio_err = $id = '';
$bodyweight = ['', ''];
$cardio = ['', ''];

// if the page is visited with a proper request
if (isset($_POST['updateExercise'])) {
    //get the submitted id
    $id= $_POST['id'];
    $dbcon = Database::GetDb();
    $ex = new Exercise();
    $exercise = $ex->GetById($dbcon, $id);

    $name = $exercise->name;

    if($exercise->body_weight == 1)
    {
    ...
    <div class="container">
        <form method="post" action="">
            ...
            <div class="form-group">
                ...
                <input type="submit" value="Update Exercise" name="uptExercise" class="btn btn-warning"/>
            </div>
