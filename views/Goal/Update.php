...
if(isset($_SESSION['id'])){
    ...
    $goal = new Goal();
    //$gid = $_POST['goalid'];
    $updateGoal = $goal->GetById(Database::GetDb(),(int)$_POST['goalid'])[0];
                ...
                <div class="card-body">
                    <form method="post" action="">
                        <div class="form-group">
                            ...
                            value="<?php echo $updateGoal->intent;?>"
                            >
                            <small id="intentHelp" class="form-text text-muted">It is ok to make small goals first!</small>
                            <p class="text-danger"><?php echo $intentErr;?></p>
                            
                        </div>
                        <div class="form-group">
                            <label for="startDate">Start date: </label>
                            <input type="date" class="form-control" id="startDate" name="startDate" value="<?php echo $updateGoal->start_date;?>">
                            <p class="text-danger"><?php echo $startErr;?></p>
                        </div>
                        <div class="form-group">
                            <label for="endDate">End date: </label>
                            <input type="date" class="form-control" id="endDate" name="endDate" value="<?php echo $updateGoal->end_date;?>">
                            <p class="text-danger"><?php echo $endErr;?></p>
                        </div>
                        <p class="text-danger"><?php echo $dateErr;?></p>
                        <input type="hidden" name="goalid" value="<?php echo $updateGoal->id;?>">

                        <button type="submit" name="updateGoal" class="btn btn-primary">Update</button>
                    </form>
                    <form method="post" action="Delete.php">
                        <input type="hidden" name="goalid" value="<?php echo $updateGoal->id;?>">
                        <button type="submit" name="deleteGoal" class="btn btn-danger">Delete</button>
                    </form>
                </div>
        </div><?php
